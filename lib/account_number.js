export class AccountNumber {
	static fromArray(accountNumberArray) {
		return new AccountNumber(accountNumberArray.map(Number))
	}
	...
	get readable() {
		return !this.accountNumberArray.some(Number.isNaN)
	}
	...
	toString() {
		let separator = '	'
		let accountNumberString = this.accountNumberArray
			.map(digit => Number.isNaN(digit) ? '?' : digit) //Replace unreadable digits
			.join('')
		return (accountNumberString + separator + this.status).trim()
	}
